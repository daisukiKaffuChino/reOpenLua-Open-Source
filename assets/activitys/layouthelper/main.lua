...
function none()
  local old=curr[tostring(_sfd_dlg.Title)]
  curr[tostring(_sfd_dlg.Title)]=nil
  --sfd_dlg.hide()
  local s,l=pcall(loadlayout2,layout.main,{})
  if s then
    activity.setContentView(l)
   else
    curr[tostring(_sfd_dlg.Title)]=old
    print(l)
  end
end
...
local th=LuaThemeUtil(this)
--输入属性对话框
sfd_dlg=AlertDialogBuilder(activity)
fld=EditText(activity)
sfd_dlg.setView(fld)
sfd_dlg.setTitle("设置内容")
sfd_dlg.setPositiveButton("确定",{onClick=ok})
sfd_dlg.setNegativeButton("取消",nil)
sfd_dlg.setNeutralButton("无",{onClick=none})

_sfd_dlg=sfd_dlg.create()

posi = _sfd_dlg.getButton(DialogInterface.BUTTON_POSITIVE)
nega = _sfd_dlg.getButton(DialogInterface.BUTTON_NEGATIVE)
tra = _sfd_dlg.getButton(DialogInterface.BUTTON_NEUTRAL)
posi.setTextColor(th.getColorPrimary())
nega.setTextColor(th.getColorPrimary())
tra.setTextColor(th.getColorPrimary())
...
function onStart()
  ...
  activity.getWindow().setNavigationBarColor(0xff3f51b5)
end
